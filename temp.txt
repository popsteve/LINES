...
func _ready():
	...
	# 连接UI信号
	game_ui.connect("line_color_selected", self, "_on_line_color_selected")
	game_ui.connect("train_added_to_line", self, "_on_train_added_to_line")
	
	# 连接游戏管理器信号
	game_manager.connect("station_added", self, "_on_station_added")
...
func _on_station_added(station: Station):
	...
	timer.wait_time = 2.0
	add_child(timer)
	timer.connect("timeout", self, "_on_highlight_timeout", [station, timer])
	timer.start()
